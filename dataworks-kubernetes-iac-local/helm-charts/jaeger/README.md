...
To add auth to jaegder UI:

```bash

# create a password file name "auth" containing username "dw-jaeger"  & password 
$ htpasswd -c auth dw-jaeger
New password: <bar>
New password:
Re-type new password:
Adding password for user foo

# create secret named basic-auth-jaeger 
$ kubectl create secret generic basic-auth-jaeger --from-file=auth
...
# add anotation to ingress menifest
annotations:
    ...
    nginx.ingress.kubernetes.io/auth-realm: 'Authentication Required - dataworks jaeger'
